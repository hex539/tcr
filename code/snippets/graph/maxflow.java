import java.util.*; import static java.lang.Math.*; // bring in globals like min,max,round
class MaxFlow {
  static class Vertex {
    Map<Vertex,Integer> mEdges = new HashMap<>(); int mId;
    public Vertex(int id) {mId = id;}
    @Override public int hashCode() {return mId;}
    @Override public boolean equals(Object a) {return mId == ((Vertex) a).mId;}
  }
  List<Vertex> mVerts;
  ...
  int dfs(Vertex s,Vertex t,Map<Vertex,Integer> dist,int cap) {
    ...
    for (Map.Entry<Vertex,Integer> y : todo) upEdge(y.getKey(),s,y.getValue());
    if (res == 0) dist.put(s, -2);
    return res;
  }
  ...
  int getFlow(Vertex s,Vertex t) {
    ...
    while (bfs(s,t,done)) res += dfs(s,t,done,1<<30);
    return res;
  }
}
public class maxflow {
  public static void main(String[] args) {
    ...
    m.addEdge(0,1, 2); m.addEdge(0,2, 5); // 2 .__.__. 5
    m.addEdge(1,3, 3); m.addEdge(2,3, 2); //  3 \_._/ 2
